...
U8 INT(LC* lc, bool ri) {
  ...
  switch (val) {
    ...
    case INT_VIDEO_WRITE: {
      GGtype(&(lc->gg), lc->renderer, lc->reg[DT].word, (U8)lc->reg[AC].word);
      break;
    }
    case INT_VIDEO_FLUSH: {
      GGpage(&(lc->gg), lc->renderer);
      break;
    }
    case INT_RAND: {
...
U8 CMP11(LC* lc) {  // 0D
  ...
  if (((I16)lc->reg[rc.x].word - lc->reg[rc.y].word) < 0) SET_NF(lc->PS);
  else RESET_NF(lc->PS);
...
U8 CMP10(LC* lc) {  // 0E
  ...
  if (((I16)(lc->reg[lc->mem[lc->PC+1]].word - Read24(lc, lc->PC+2)) < 0)) SET_NF(lc->PS);
  else RESET_NF(lc->PS);
...
U8 POW11(LC* lc) {
  lcrc_t rc = ReadRegClust(lc->mem[lc->PC+1]);
  lc->reg[rc.x].word = (U16)pow(lc->reg[rc.x].word, lc->reg[rc.y].word);
  lc->PC += 2;
  return 0;
}
...
U8 CMPpi(LC* lc) {
  ...
  if (((I16)(lc->mem[lc->reg[lc->mem[lc->PC+1]].word] - Read24(lc, lc->PC+2)) < 0)) SET_NF(lc->PS);
  else RESET_NF(lc->PS);
  lc->PC += 5;
  return 0;
}
...
U8 LWc(LC* lc) {
  lcrc_t rc = ReadRegClust(lc->mem[lc->PC+1]);
  lc->reg[rc.y].word = ReadWord(lc, lc->reg[rc.x].word);
  lc->reg[rc.x].word += 2;
  lc->PC += 2;
  return 0;
}
...
U8 (*INSTS[256])(LC*) = {
  ...
  &LDA1 , &LDB1 , &LDC1 , &LDD1 , &LDT1 , &LDI1 , &LDTP1, &LDBP1, &UNK  , &UNK  , &UNK  , &UNK  , &UNK  , &UNK  , &UNK  , &CLC  ,
  ...
  &SBc  , &LBc  , &SWc  , &LWc  , &SHc  , &LHc  , &UNK  , &UNK  , &CIF  , &CFI  , &ADDF , &SUBF , &MULF , &DIVF , &NEGF , &POWF
};
  ...
  execloop:
    ...
    if (lc->mem[lc->PC]==0) {
      ...
      for (U8 i = 0; i < RECOVERY_SIZE; i++) {
        printf("%06X ", govnu_recovery[(RECOVERY_SIZE-i+ptr)%RECOVERY_SIZE]);
        if (i%32==31)printf("\n");
      }
    }
