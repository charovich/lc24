#!/usr/bin/env bash
# GovnPy wrapper
# Copyright (C) 2025 t.me/pyproman & Chara
#
...
crash() {
    ...
    printf "\x1b[31mLC24 exited with error:\x1b[0m %s\n" $1
}
gpy-run() {
    # Parse command line
    target="2"
    ...
    case $target in
        1)
            printf "\x1b[33mBro this is not boxlang we have regstack not push pop spam\x1b[0m\n"
    ...
    python3 $LOST/asm/las progs/compiled/$module.s progs/compiled/$module.bin
    python3 $LOST/asm/las -export progs/compiled/$module.s progs/compiled/$module.sym || true
    case $mode in
        cli)
