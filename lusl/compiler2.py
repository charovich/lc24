#!/bin/python3
# Lusl compiler verdton 2
# Copyright (C) 2025 t.me/pyproman
# Copyright (C) 2025 Lost Project
#
...
def emit_expr(ctx, node):
    match node:
        case ast.Constant():
            match node.value:
                case str():
                    dtd = f"str{len(ctx[CTX_GLOB])}"
                    ctx[CTX_GLOB][dtd] = node.value + "\x00"
                    ctx[CTX_RSF].append(...)
                    ctx[CTX_FD].write(f"  mov {getreg(ctx[CTX_RSF])} {dtd}\n")
                case int():
                    ctx[CTX_RSF].append(...)
        ...
        case ast.Name():
            if node.id in ctx[CTX_LOC]:
                ...
                ctx[CTX_FD].write(f"  mov %dt {ctx[CTX_LOC][node.id]}\n"
                                  f"  add %dt %bp\n" +
                                 (f"  xchg %ac {reg}\n" if reg != "%ac" else "") +
                                  f"  lodsw\n" +
                                 (f"  xchg %ac {reg}\n" if reg != "%ac" else ""))
        ...
        case ast.Subscript():
            ...
            ctx[CTX_FD].write(f"  mov %dt {getreg(ctx[CTX_RSF])}\n"
                              f"  add %dt {getreg(ctx[CTX_RSF], -2)}\n"
                              f"  mov {getreg(ctx[CTX_RSF], -2)} *%dt\n")
            ctx[CTX_RSF].pop()
        case ast.BoolOp():
        ...
        case ast.BinOp():
            if isinstance(node.right, ast.Constant):
                op = {
                    ...
                    ast.Sub: "sub {} {}",
                    ast.Mult: "mul {} {}",
                    ast.Mod: "div {0} {1} mov {0} %dc",
                    ast.BitXor: "mov %dc {0} or %dc {1} and {0} {1} sub {0} %dc",
                    ast.BitAnd: "mov %dc {1} and {0} %dc",
                }[type(node.op)]
                emit_expr(ctx, node.left)
            ...
            op = {
                ...
                ast.Mult: "mul {} {}",
                #ast.Mod: "div {0} {1} mov {0} %dc",
                ast.Mod: "mov %dt {0} div {0} {1} mul {0} {1} sub %dt {0} mov {0} %dt",
                ast.BitXor: "mov %dc {0} or %dc {1} and {0} {1} sub {0} %dc",
                ast.BitAnd: "and {} {}"
            }[type(node.op)]
            emit_expr(ctx, node.left)
        ...
        case _:
            raise ValueError(f"Unsupported expresdton: {type(node)}")
def emit_block(ctx, node):
    match node:
        case ast.Assign():
            if len(node.targets) != 1:
                raise ValueError("Multiple asdtgnments are unsupported")
            match node.targets[0]:
                case ast.Name(id=vname):
                    stack = []
                    emit_expr(ctx + (stack,), node.value)
                    ctx[CTX_FD].write(f"  mov %dt {ctx[CTX_LOC][vname]}\n"
                                    f"  add %dt %bp\n"
                ...
                case ast.Subscript():
                    ...
                    ctx[CTX_FD].write(f"  mov %dt {getreg(stack)}\n"
                                    f"  add %dt {getreg(stack, -2)}\n"
        ...
        case ast.Assign():
            ...
            if isinstance(node.targets[0], ast.Name):
                ...
                if vname not in ctx[CTX_LOC]:
                    ctx[CTX_LOC][vname] = len(ctx[CTX_LOC]) * 3
        case ast.If():
            for i in node.body:
                alloc_block(ctx, i)
            for i in node.orelse:
...
def emit_global(ctx, node):
    match node:
        case ast.Import():
            for i in node.names:
                if i.name.startswith("lusl."):
                    file = f"libs/{i.name[5:]}.s"
                    with open(file) as m:
                        ctx[CTX_FD].write(f"; {file}\n")
                        ctx[CTX_FD].write(m.read())
                        ctx[CTX_FD].write("\n")
                elif i.name.startswith("gdi."):
                    file = f"libs/gdi/{i.name[4:]}.s"
                    with open(file) as m:
                        ctx[CTX_FD].write(f"; {file}\n")
                        ctx[CTX_FD].write(m.read())
                        ctx[CTX_FD].write("\n")
        ...
        case ast.FunctionDef():
            ...
            for k in rsf.keys():
                rsf[k] = len(rsf) * 3 - rsf[k] - 3
            frame = 3*(len(rsf)-len(node.args.args)-1)
            ...
            for i in node.body:
                emit_block(nctx, i)
            ctx[CTX_FD].write(f"  psh 0\n"
                              f".ret:\n"
                              f"  pop %dc\n" # ret val
                              f"  mov %ac {frame}\n"
                              f"  add %sp %ac\n"
                              f"  pop %ac\n"
                              f"  mov %bp {3*len(node.args.args)}\n"
                              f"  add %sp %bp\n"
                              f"  psh %dc\n"
                              f"  psh %ac\n"
                              f"  ret\n")
        ...
        case ast.Assign():
            ...
            match node.value:
                ...
                case ast.Call(func=ast.Name(id="spr16"), args=[ast.Constant(value=str())]):
                    ...
                    ctx[CTX_FD].write("\n")
                case _:
                    raise ValueError(f"Unkown asdtgnment type {type(node.value)}")
        case _:
            print(f"Skipping {type(node)}, FIXME")

parser = argparse.ArgumentParser(prog='lusl', description='Lusl compiler')
parser.add_argument('source', type=str, help='Lusl source code')
parser.add_argument('output', type=str, help='Lusl file name')
